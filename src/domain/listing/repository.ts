...
export class ListingRepository extends Context.Tag("ListingRepository")<
  ListingRepository,
  {
    ...
    ) => Effect.Effect<Listing, NotFoundError | UnauthorizedError | DatabaseError>;
    ...
    listByUser: (
      userId: string,
    ) => Effect.Effect<Listing[], DatabaseError>;
  }
>() {}

// --- Layer Drizzle (production) ---

export const ListingRepositoryDrizzle = Layer.succeed(
  ListingRepository,
  {
    getById: (id) =>
      Effect.tryPromise({
        try: () =>
          db.select().from(listing).where(eq(listing.id, id)).then((rows) => rows[0]),
    ...
    create: (input) => {
      if (input.price < 0) {
        return Effect.fail(
          new ValidationError({ field: "price", message: "Le prix ne peut pas être négatif" }),
        );
      }
      ...
      Effect.tryPromise({
        ...
        Effect.flatMap((row) =>
          row.userId !== requestingUserId
            ? Effect.fail(new UnauthorizedError({ reason: "Tu n'es pas le vendeur" }))
            : Effect.succeed(row),
        ),
        Effect.flatMap(() =>
          Effect.tryPromise({
    ...
    listByUser: (userId) =>
      Effect.tryPromise({
        try: () =>
          db.select().from(listing).where(eq(listing.userId, userId)),
        catch: (cause) => new DatabaseError({ cause }),
      }),
  },
);
